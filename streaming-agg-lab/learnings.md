...
**Date:** 2026-07-08 · **Concept:** *Can I trust a streaming aggregate — and what makes it wrong vs right?*
**Format:** predict → run → compare, driven hands-on. Full evidence in [`WHY.md`](./WHY.md).
...
```mermaid
flowchart LR
    P["Node producer<br/>(kafkajs)"] --> T1["Kafka topic: events<br/>3 partitions"]
    T1 --> F["Flink SQL<br/>10s tumbling event-time<br/>window · COUNT by type"]
    F --> T2["Kafka topic: agg-results"]
    T2 --> S["Node sink<br/>(consumer → INSERT)"]
    S --> CH[("ClickHouse<br/>OLAP store")]
    CH --> Q["Node query<br/>(dashboard stand-in)"]
```
...
```mermaid
flowchart TD
    A["Event arrives at Flink"] --> B{"event_time still inside<br/>an OPEN window?"}
    B -->|yes| C["Counted correctly"]
    B -->|"no — watermark already<br/>closed that window"| D["DROPPED · silent loss<br/>(only numLateRecordsDropped hints)"]
    D --> E{"widen watermark delay?"}
    E -->|"yes (e.g. 2s → 30s)"| F["Now counted…<br/>but every window publishes<br/>~30s later (stale dashboard)"]
    E -->|no| G["Lost forever"]
```
...
```mermaid
flowchart TD
    X["Flink crashes / restarts"] --> Y{"checkpointing on?"}
    Y -->|no| Z["LOSS · in-flight state gone,<br/>outage events skipped<br/>(loss ∝ total downtime)"]
    Y -->|yes| W{"idempotent sink?"}
    W -->|no| V["DUPLICATES · windows that fired<br/>after last checkpoint are replayed<br/>and written twice (rows=2)"]
    W -->|yes| U["CORRECT · exactly-once<br/>at the OLAP store"]
```
...
```mermaid
flowchart TD
    A["Input rate >> Flink throughput"] --> B["Consumer lag grows UNBOUNDED"]
    B --> C["Aggregate falls behind wall-clock (stale)"]
    C --> D{"load subsides before<br/>Kafka retention expires?"}
    D -->|yes| E["Recovers · no loss<br/>(freshness failure only)"]
    D -->|no| F["Data loss · retention exceeded"]
    B --> G["Skew: one HOT partition caps throughput —<br/>adding consumers cannot help"]
```
